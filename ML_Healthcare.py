...
def plot_op(dataset_name):
    ...
    if dataset_name == "Heart Attack":
        col1.write(Y)
        sns.countplot(Y, palette='gist_heat',ax=ax)
        col2.pyplot(fig)
...
def compute(Y_pred,Y_test):
    ...
    ax.scatter(range(len(Y_pred)), Y_pred, color="yellow", lw=5, label="Predictions")
    ax.scatter(range(len(Y_test)), Y_test, color="red", label="Actual")
    ax.set_title("Prediction Values vs Real Values")
    ax.legend()
    ax.grid(True)
    c1.pyplot(fig)
    #Confusion Matrix
    ...
    sns.heatmap(df_cm, annot=True, cmap='Pastel1', linewidths=2, fmt='d', ax=ax)  # Pass the axes to the plot
    ax.set_title("Confusion Matrix", fontsize=15)
    ax.set_xlabel("Predicted")
    ax.set_ylabel("True")
    c2.pyplot(fig)  # Pass the figure
